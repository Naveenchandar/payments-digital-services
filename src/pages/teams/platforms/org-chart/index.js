...
export class OrganizationChart extends React.Component {
  ...
  template({ photo, name, post, phone, mail, subpost, postmain }) {
    const renderPhoto = (picture) => {
      if (!picture) return "";
      return `<div class="dhx_diagram_template_a__picture" style="background-image: url(${picture});background-position:top; background-size: cover;"></div>`
    }
    ...
    const renderPostMain = (postmain) => {
      if (!postmain) return "";
      return `<span class="dhx_diagram_template_a__text">${postmain || ""}</span>`
    }
    return `
          <div class="dhx_diagram_template_a_box dhx_diagram_template_a">
            <div class="dhx_item_header" style="width:400px;"></div>
